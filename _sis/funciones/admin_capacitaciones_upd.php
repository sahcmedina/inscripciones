...
if (isset($_POST["idt1"]))          { $idt1   = $_POST["idt1"];          } else { $idt1   = '';	}
 
if (isset($_POST["tema_2_titulo"])) { $t2_tit = $_POST["tema_2_titulo"]; } else { $t2_tit = '';	}
if (isset($_POST["idt2"]))          { $idt2   = $_POST["idt2"];          } else { $idt2   = '';	}

if (isset($_POST["tema_3_titulo"])) { $t3_tit = $_POST["tema_3_titulo"]; } else { $t3_tit = '';	}
if (isset($_POST["idt3"]))          { $idt3   = $_POST["idt3"];          } else { $idt3   = '';	}

if (isset($_POST["tema_4_titulo"])) { $t4_tit = $_POST["tema_4_titulo"]; } else { $t4_tit = '';	}
if (isset($_POST["idt4"]))          { $idt4   = $_POST["idt4"];          } else { $idt4   = '';	}

if (isset($_POST["tema_5_titulo"])) { $t5_tit = $_POST["tema_5_titulo"]; } else { $t5_tit = '';	} 
if (isset($_POST["idt5"]))          { $idt5   = $_POST["idt5"];          } else { $idt5   = '';	}

if (isset($_POST["check"])) { $check = $_POST["check"]; } else { $check = [];	}  // Aca viene tododl oque hay que borrar
...
switch($opcion){
	...
	case 'falta_img':
		$_SESSION['var_retorno_']= 'curso_add_er';	$_SESSION['msj_retorno_']= 'Falta seleccionar una imagen para la capacitación'; break;
	...
	case 'err_formato_fecha':
		$_SESSION['var_retorno_']= 'curso_add_er';	$_SESSION['msj_retorno_']= 'Por favor ingrese fechas reales ';			 break;
	case 'error_fechas_c':
