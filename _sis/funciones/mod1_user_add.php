...
if (isset($_SESSION["var_id_cli"]))     { $id_cli=$_SESSION["var_id_cli"];  } else {  $id_cli= '';  }
...
if (isset($_SESSION["var_pas"]))        { $pas=$_SESSION["var_pas"];        } else {  $pas= '';     }
                                                                                                                    ...
                                                                                                                    $opc= 'ok';
if($nom=='' OR $ape=='' OR $dni=='' OR $tel=='' OR $email=='' OR $pas=='' OR $perf=='' OR $usu=='' OR $id_cli=='')  $opc= 'er';

switch($opc){

	case 'ok':
                include_once('./usuario.php');	$U = new Usuario();                	
                $next_user  = $U->add_usuario_admin($dni, $usu, $pas, $id_cli);						
