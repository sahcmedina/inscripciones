...
if($archivos[0] ==''  )      {$op='er'; $falta.='Falta Reglamento español. \n';}
if($archivos[1] ==''  )      {$op='er'; $falta.='Falta Reglamento inglés. \n';}

if($falta == ''){$op='ok'; }else{ $op='er'; }

switch($op){
	case 'er': $_SESSION['var_retorno_']= 'add_con_er'; $_SESSION['msj_retorno_']= 'Datos Faltantes.'.$falta; break;
	case 'ok':
		$nombres = array(); $nominput = array(); $actuales = array();
		...
		$add = $Con->add($habilitar, $deshabilitar, $mostrar_jurado, $mostrar_ganadores, $reglamento_es, $reglamento_en);
		if($add){
			$_SESSION['var_retorno_']= 'add_con_ok';  $_SESSION['msj_retorno_']= 'Los parámetros se Agregaron Correctamente.';
		}else{
			$_SESSION['var_retorno_']= 'add_con_er'; $_SESSION['msj_retorno_']= 'No se pudo Agregar los parámetros.';
		}
	break;
}
?>
