...
switch($opc){
	...
	case 'ok':	include_once('./mod4_ri_agenda.php');	 $RondAg = new RI_Agenda();
				$arr    = array();
				$arr    = $RondAg->gets_config($id);
				$del_v1 = true;	
				$del_v2 = true;	
				if($arr[0]['dia1'] == 1){ $del_v1 = $RondAg->aux_delTabla($arr[0]['view1']); }
				if($arr[0]['dia2'] == 1){ $del_v2 = $RondAg->aux_delTabla($arr[0]['view2']); }	 						
