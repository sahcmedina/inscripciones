...
switch($opc){
    case 'ok':	
        ...
        $add_det = $Con->add_conferencia($disertante, $organismo, $modalidad, $cupo, $estado, $f_update, $id_user);
        if($add_det){
            $ult_id_foro = $Con->get_last_id();
            // guardo cabecera. Tbl eventos
            $add_cab = $Con->add_evento($tipo, $ult_id_foro, $titulo, $fecha, $hora, $lugar, $insc_inicio, $insc_final, $f_create, $id_user);
