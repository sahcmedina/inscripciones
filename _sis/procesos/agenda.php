<?php
	...
	// ------------------------------ FUNCION ------------------------------ //	
	## 1 - Se lee la tabla "ronda_inscriptos_prod" y llenamos la tabla "ronda_union_cv"
	...
	// a- averiguo disponibilidad de tablas para trabajar. 
	$arr_disponib = array();
	...
	// b- asigno las tablas a usar && inicializo
	if($hay!= 0){
		...
		// no hay tabla desocupada para correr el proceso
	...
	// c- corro procesos 1, 2, y 3
	// $del = $RN->aux_vaciar_union_cv();
	if($hay !=0){		$correr_proc_1 = 'si';	$correr_proc_2 = 'si';	$correr_proc_3 = 'si';	}
	else{				$correr_proc_1 = 'no';	$correr_proc_2 = 'no';	$correr_proc_3 = 'no';  }
	
	$t_1 = 0;
	$t_2 = 0;

	// ----------------------------------------------------------------------------------------------
	// 1 - Completar tabla: eventos_ronda_neg_union_cv ----------------------------------------------
	// ----------------------------------------------------------------------------------------------
	...
	if($correr_proc_1 == 'si'){
		...
		// Para c/Prod traigo #C y #V
		for($i=0 ; $i<count($arr_prod) ; $i++){
			$knt_c = $RN->get_cant($arr_prod[$i]['id'], 'c', $id_rn);
			$knt_v = $RN->get_cant($arr_prod[$i]['id'], 'v', $id_rn); 
			...
			if($knt_c >0 && $knt_v >0){
				...
				$arr_c = $RN->gets_cv_prod('c', $arr_prod[$i]['id'], $id_rn); // Emp q compran X producto
				$arr_v = $RN->gets_cv_prod('v', $arr_prod[$i]['id'], $id_rn); // Emp q venden  X producto 
	...
	// - libero tablas
	$upd_libTabla= $RondAg->upd_liberar_tabla($hay);
	// Ctrl
	if($upd_libTabla)	$proceso3= true; else $proceso3= false;
	...
	if($op=='ok' && !$proceso3){		            $op= 'er';   $msj='Al liberar tabla.'; }
