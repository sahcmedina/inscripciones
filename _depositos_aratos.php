<?php
	...
	// ------------------------------ FUNCION ------------------------------ //			
	include('./funciones/mod2_aratos.php');       $Aratos= new Aratos();
	include('./funciones/mod2_depositos.php');    $Depo  = new Depositos();

	$arr_aratos = array(); 
	$arr_aratos = $Aratos->gets(); //echo 'ACA: '.count($arr_aratos);die();
...
$(document).ready(function(){  
	$('#modal_upd').on('show.bs.modal', function (event) {    
		  ...
		  modal.find('.modal-body #dep_').val(dep)
		  ...
		  $("#ver_deposito").delay(10).queue(function(n) { 
				$.ajax({
						type: "POST",
						url:  "./funciones/mod2_ajax_depo_upd_arato.php",
						data: "dep="+dep,
						dataType: "html",
						error: function(){
    ...
    $("#validar_upd").click(function(){
		...
		nom  = $("#nombre").val();			
	  	...
	  	$("#mostrar_validar_upd").delay(15).queue(function(n) {                                                 
            $.ajax({
                type: "POST",
                url: "./funciones/mod2_ajax_validar_upd_arato.php",                                                                                                                                                                                                    
                data: "id="+id+"&cod="+cod+"&cod_ant="+codi_ant+"&depo_="+depo_+"&usuario="+usu+"&nom="+nom,     
              	dataType: "html",
    ...
    $("#validar_del").click(function(){
		id = $("#id_del").val();			
	  	...
	  	$("#mostrar_validar_del").delay(15).queue(function(n) {                                                 
            $.ajax({
                type: "POST",
                url: "./funciones/mod2_ajax_validar_del_arato.php",                                                                                                                                                                                                    
                data: "id="+id,     
              	dataType: "html",
                error: function(){	        alert("error petición ajax");           		},
        ...
        <div id="content" class="main-content">
            <div class="layout-px-spacing">
                ...
                <div class="middle-content container-xxl p-0">
                                    ...
                                    <div class="page-header">
                                        ...
                                        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                                            <ol class="breadcrumb">
                                                ...
                                                <li class="breadcrumb-item active" aria-current="page">Aratos</li>
                            ...
                            <div class="widget-content widget-content-area br-8">
                                <table id="dt_" class="table dt-table-hover" style="width:100%">
                                    ...
                                    <?php
                                        ...
                                        echo $tabla;
                                        for($j=0 ; is_array($arr_aratos) && $j<count($arr_aratos) ; $j++){
                                            $cur  = $arr_aratos[$j];
                                        
                                            $btn_del = ' <button data-bs-toggle="modal" data-bs-target="#modal_del" 
                                                            data-id="'.$cur['id'].'" data-codigo="'.$cur['codigo'].'"
                                                            class="btn btn-outline-danger btn-icon mb-2 me-4" title="Finalizar un Arato" >
                                                            <i class="bi bi-trash" style="font-size: 1rem;"></i>
                                                            </button>';

                                            $btn_mdf = '<button data-bs-toggle="modal" data-bs-target="#modal_upd" 
                                                           data-id="'.$cur['id'].'" data-codigo_ant="'.$cur['codigo'].'" data-dep="'.$cur['fk_deposito'].'" data-nom="'.$cur['nombre'].'"
                                            ...
                                            . '<td style="text-align:center">'. $cur['nom_dep'].' - P170'."</td>\n"
                            ...
                            <form name="form_del_user" id="form_del_user" class="form-horizontal validate" method="post" action="#" enctype="multipart/form-data" >
                                ...
                                <div class="modal-body with-padding">					
                                    <div class="form-group-sm">
                                        <div class="row">
                                            <label><i class="icon-warning"></i> La función permite administrar los diferentes Aratos de los Depositos de la empresa. También permite: </label>
                                            <br/><br/>
                    ...
                    <!-- Modal: Borrar / Finalizar -->
                    <div id="modal_del" class="modal animated fadeInDown" tabindex="-1" role="dialog">
                        <div class="modal-dialog modal-content" >
                            <div class="modal-content">
                            
                            <div class="modal-header"><h6 class="modal-title"> Finalizar Arato </h6></div>
                    ...
                    <div class="modal-dialog modal-xl">
                            ...
                            <div class="modal-header"><h6 class="modal-title"> Agregar Arato </h6></div>
                                    ...
                                    <div class="form-group-sm">                                    
                                        <div class="row">
                                            ...
                                            <div class="col-md-4">
                                                ...
                                                <select id="dep" name="dep" class="form-select dep form-control-sm" tabindex="3"><?php
                    ...
                    <div id="modal_upd" class="modal animated fadeInDown" tabindex="-1" role="dialog">
                        <div class="modal-dialog modal-xl">
                            ...
                            <div class="modal-header"><h6 class="modal-title"> Modificar Arato </h6></div>
